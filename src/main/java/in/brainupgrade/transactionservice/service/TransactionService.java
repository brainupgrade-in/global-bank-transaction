...
public class TransactionService implements TransactionServiceInterface {
	...
	public boolean makeTransfer(String token, TransactionInput transactionInput) throws MinimumBalanceException {
		...
		Boolean check = (Boolean) ruleFeign.evaluate(new RulesInput(sourceAccount.getAccountId(), sourceAccount.getCurrentBalance(), transactionInput.getAmount())).getBody();
		//String name = body.toString();
		if (check.booleanValue() == false) throw new MinimumBalanceException("Minimum Balance 1000 should be maintaind");
	...
	public boolean makeWithdraw(String token, AccountInput accountInput) {
		...
		Boolean check = (Boolean) ruleFeign.evaluate(new RulesInput(accountInput.getAccountId(), sourceAccount.getCurrentBalance(), accountInput.getAmount())).getBody();
